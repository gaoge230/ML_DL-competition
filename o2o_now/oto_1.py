...
def get_simple_feature(label_field):
    ...
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data, index=keys, values='cnt', aggfunc=len)
    pivot = pd.DataFrame(pivot).rename(columns={'cnt': prefixs + 'user_distance_receive'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
    #用户特定商户的个数
    keys = ['User_id', 'Merchant_id']
    prefixs = 'simple_' + '_'.join(keys) + '_'
    pivot = pd.pivot_table(data, index=keys, values='cnt', aggfunc=len)
    pivot = pd.DataFrame(pivot).rename(columns={'cnt': prefixs + 'user_Merchant_id_receive'}).reset_index()
    feature = pd.merge(feature, pivot, on=keys, how='left')
...
if __name__ == '__main__':
    ...
    # off_test_1 = get_simple_feature(off_test).drop(['Discount_rate','Distance','date_received'],axis=1)
    # result,feat_importance =model_xgb(big.train,off_test_1)
    # #最终结果
