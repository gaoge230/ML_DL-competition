...
if __name__ == '__main__':
    ...
    #
    # #构造训练集、验证集、测试集
    # # train = get_simple_feature(test_history_field).drop(['Discount_rate','Distance','Date','date_received','date'], axis=1)
    # # validate = get_simple_feature(validate_history_field).drop(['Discount_rate','Distance','Date','date_received','date'], axis=1)
    # # test = get_simple_feature(test_history_field).drop(['Discount_rate','Distance','Date','date_received','date'], axis=1)
    # train,validate,test,u_feat,w_feat,c_feat= oto_feature_0.get_feature(train_history_field,validate_history_field,test_history_field)
    #
    ...
    # off_test = pd.merge(off_test, u_feat, on='User_id', how='left')
    # off_test.fillna(0, downcast='infer', inplace=True)
    # off_test = pd.merge(off_test, w_feat, on='Merchant_id', how='left')
    # off_test.fillna(0, downcast='infer', inplace=True)
    # off_test = pd.merge(off_test, c_feat, on='Coupon_id', how='left')
    # off_test.fillna(0, downcast='infer', inplace=True)
    # off_test_1 = oto_feature_0.get_simple_feature(off_test).drop(['Discount_rate','Distance','date_received'],axis=1)
